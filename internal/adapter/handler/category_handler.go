...
type CategoryHandler interface {
	...
	EditCategoryID(c *fiber.Ctx) error
...
func (ch *categoryHandler) GetCategoryFE(c *fiber.Ctx) error {
	...
	for _, result := range results {
		categoryResponse := response.SuccessCategoryResponse{
			...
			Title:         result.Title,
			Slug:          result.Slug,
			CreatedByName: result.User.Nama,
		}
...
// EditCategoryID implements CategoryHandler.
func (ch *categoryHandler) EditCategoryID(c *fiber.Ctx) error {
	var req request.CategoryRequest
	claims := c.Locals("user").(*entity.JwtData)
	userID := claims.UserID
	if userID == 0 {
		code = "[HANDLER] EditCategoryID - 1"
		log.Errorw(code, err)
		errorResp.Meta.Status = false
		errorResp.Meta.Message = "unauthorized access"
	...
	err = ch.categoryService.EditCategoryID(c.Context(), reqEntity)
	if err != nil {
		code = "[HANDLER] EditCategoryID - 5"
		log.Errorw(code, err)
		errorResp.Meta.Status = false
		errorResp.Meta.Message = err.Error()
		return c.Status(fiber.StatusInternalServerError).JSON(errorResp)
...
func (ch *categoryHandler) GetCategoryByID(c *fiber.Ctx) error {
	...
	defaultSuccessResponse.Meta.Status = true
	defaultSuccessResponse.Pagination = nil
	defaultSuccessResponse.Meta.Message = "Category fetched details successfully"
