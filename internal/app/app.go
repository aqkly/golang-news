...
func RunServer() {
	...
	cagoryService := service.NewCategoryService(categoryRepo)
	contentService := service.NewContentService(contentRepo, cfg, r2Adapter)
	userService := service.NewUserService(userRepo)

	// handler
	authHandler := handler.NewAuthHandler(authService)
	categoryHandler := handler.NewCategoryHandler(cagoryService)
	...
	categoryApp.Put("/:categoryID", categoryHandler.EditCategoryID)
	...
	// start server
	log.Println("Server started on port " + cfg.App.AppPort)
	if cfg.App.AppPort == "" {
